...
class ZeroLengthCavity(tk.Component):
    ...
    def __build_cell(self):
        ...
        for i in range(int(self.num_holes)):
            ...
            y = starty
            hole_list.append(gdspy.Round((x, y), self.radius, tolerance=0.0001))
        ...
        for hole in hole_list:
            nanobeam = gdspy.boolean(nanobeam, hole, "xor")
        for hole_in in taper_list_in:
            nanobeam = gdspy.boolean(nanobeam, hole_in, "xor")
        for hole_out in taper_list_out:
            nanobeam = gdspy.boolean(nanobeam, hole_out, "xor")
...
class InlineZeroLengthCavity(tk.Component):
    """ Zero-Length Cavity Cell class that can be put in-line with a waveguide.
    ...
    def __init__(
        ...
        tk.Component.__init__(self, "InlineZeroLengthCavity", locals())
    ...
    def __build_cell(self):
        ...
        for hole_in in taper_list_in:
            nanobeam = gdspy.boolean(nanobeam, hole_in, "xor", precision=0.0001)
        for hole_out in taper_list_out:
            nanobeam = gdspy.boolean(nanobeam, hole_out, "xor", precision=0.0001)
